...
FROM student s JOIN faculty f ON s.f_id = f.f_id;


-- 2. Retrieve the advisor name along with the number of students they are 
--    advising.
SELECT f_first, f_last, COUNT(s_id)
...
SELECT f.f_id, f.f_last, SUM(cs.max_enrl)
...
SELECT cs.course_id, c.course_name, cs.max_enrl
...
--    comment in SQL file
INSERT INTO course_section (c_sec_id, course_id, term_id, sec_num, f_id, loc_id, 
                            max_enrl)
VALUES (15, 6, 6, 1, 1, NULL, 35);
...
SELECT s2.s_last
FROM student s1 JOIN student s2 ON s1.f_id = s2.f_id
